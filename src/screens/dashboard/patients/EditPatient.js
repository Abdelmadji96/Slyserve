...
import {doctorUpdatePatient} from '../../../api/doctors';
...
const EditPatient = ({navigation, route, application, user, role, token}) => {
  const {drawer} = useTheme();
  const [firstName, setFirstName] = useState(route.params.patient.prenom);
  ...
  const [lastName, setLastName] = useState(route.params.patient.nom);
  ...
  const handleUpdatePatient = async () => {
    ...
    try {
      const response = await doctorUpdatePatient(
  ...
  const isGenderChanged = () => {
    let changed = false;
    if (route.params.patient.genre == 'H') {
      if (gender == 2) {
        changed = true;
      }
    } else {
  ...
  const allValidate = () => {
    if (
      ...
      if (
        firstName == route.params.patient.prenom &&
        lastName == route.params.patient.nom &&
        !isGenderChanged() &&
        address == route.params.patient.nom_de_rue &&
        phone == route.params.patient.telephone &&
        wilaya == route.params.patient.wilaya_id &&
        commune == route.params.patient.commune_id &&
        displayDate(dateOfBirth) ==
          displayDate(new Date(route.params.patient.date_de_naissance))
      ) {
  ...
  useEffect(() => {
    getWilayas();
    getCommunes(route.params.patient.wilaya_id);
  }, []);
...
export default connect(mapStateProps, null)(EditPatient);
