...
import {doctorDeletePatient} from '../../api/doctors';
...
const Patient = ({
  ...
  const handleDeletePatient = async () => {
    try {
      const response = await doctorDeletePatient(patient.id, token);
      alert(JSON.stringify(response));
      if (response) {
        if (response['message'] == 'success') {
          setPatients(patients.filter(item => item.id !== patient.id));
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        styles.container,
        {marginBottom: index == patients.length - 1 ? 20 : 0},
      ]}
      ...
      <View style={styles.row}>
        <Feather
          ...
          style={styles.icon}
        />
        <Text>{patient.date_de_naissance.substr(0, 10)}</Text>
      </View>
      <View style={styles.row}>
      ...
      <View style={styles.row}>
        <Feather
          ...
          style={styles.icon}
        />
        <Text>{patient.telephone.substr(0, 10)}</Text>
      </View>
      <Modal
        transparent={true}
...
export default connect(mapStateProps, mapDispatchProps)(Patient);
