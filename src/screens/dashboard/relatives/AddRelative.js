...
import {patientAddRelative} from '../../../api/patients';
...
const AddRelative = ({navigation, route, application, user, token}) => {
  ...
  const handleAddRelative = async () => {
    ...
    try {
      const response = await patientAddRelative(
        firstName,
        lastName,
      ...
      <Animated.View
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {application.language.data.ADD_RELATIVE}
          </Text>
          <Feather
            name="save"
            size={30}
...
export default connect(mapStateProps, mapDispatchProps)(AddRelative);
